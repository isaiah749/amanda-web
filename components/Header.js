      ...
      <div className='flex items-center justify-between bg-gradient-to-b bg-gray-200 from-emerald-300 to-transparent py-2 md:px-10 px-2'>
          <div onClick={() => router.push('/')}>
            ...
            <ul className={`pb-3 absolute pt-12 bg-white xl:bg-transparent z-[-1] left-0 w-full desktop-nav pl-2 transition-all duration-500 ease-in md:pt-20 ${open ? 'top-10 ':'top-[-490px]'}`}>
                ...
                <div  className="md:ml-5 text-lg lg:text-lg ">
                    <li onClick={() => router.push('/')} className="text-emerald-600  hover:text-emerald-400 font-semibold mt-2  md:mt-2 lg:mt-0 cursor-pointer md:text-2xl lg:text-lg  duration-500 ">Home</li>
                </div>

                <div  className="md:ml-5 text-lg lg:text-lg ">
                    <li onClick={() => router.push('/about-dyslexia')} className="text-emerald-600  hover:text-emerald-400 font-semibold mt-4  md:mt-2 lg:mt-0 cursor-pointer md:text-2xl lg:text-lg  duration-500 ">Whats Dyslexia</li>
                </div>
            
                <div className="md:ml-5 text-lg lg:text-lg my-4 ">
                    <li onClick={() => router.push('/about')} className="text-emerald-600  hover:text-emerald-400 font-semibold my-4 md:my-2 md:text-2xl lg:text-lg lg:my-0 cursor-pointer duration-500 ">About</li>
                </div>
    
                <div className="md:ml-5 text-lg lg:text-lg my-4 ">
                    <li onClick={() => router.push('/services')} className="text-emerald-600  hover:text-emerald-400 font-semibold my-4 md:text-2xl lg:text-lg md:my-2 lg:my-0 cursor-pointer duration-500 ">Services</li>
                </div>
    
                <div className="md:ml-5 text-lg lg:text-lg my-4 ">
                    <li onClick={() => router.push('/contact')} className="text-emerald-600  hover:text-emerald-400 font-semibold my-4 md:text-2xl lg:text-lg md:my-2 lg:my-0 cursor-pointer mr-5 duration-500 ">Contact</li>
                </div>
